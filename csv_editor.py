...
class CSVEditor():
    ...
    def get_current_score_dataframe(self, users, inactive_user = None):
        scores = np.array([])
        for user in users:
            sc = 0
            if inactive_user is None or not user in inactive_user:
                sc = np.sum(self.get_user_month_score(user))
            scores = np.append(scores, sc)

        if inactive_user is not None:
            mean_score = np.round(np.sum(scores)/(len(users)-len(inactive_user)))
            for i, user in enumerate(users):
                if user in inactive_user:
                    scores[i] = mean_score
    ...
    def daily_check_in_auto(self, users, inactive_user = None):
        '''
...
if __name__ == "__main__":
    ...
    ceditor.daily_check_in_auto(constants.USERS, constants.INACTIVE)
